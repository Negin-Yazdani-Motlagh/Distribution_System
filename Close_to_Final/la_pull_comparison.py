...
def plot_comparison(df):
    ...
    # Plot Uniform Pull lines (3 horizontal lines, one per n)
    for i, n in enumerate(N_VALUES):
        # Get the constant value (same for all p)
        subset = df[(df['algorithm'] == 'Uniform Pull') & (df['n'] == n)]
        if len(subset) > 0:
            ...
            ax.axhline(y=mean_val, color=colors_uniform[i], 
                      linestyle=line_styles_uniform[i], linewidth=2.5,
                      label=f'Uniform Pull (n={n//1000}k)',
                      zorder=2)
            
            # Add shaded CI band (horizontal)
            ax.axhspan(ci_low, ci_high, alpha=0.10, color=colors_uniform[i], zorder=0)
