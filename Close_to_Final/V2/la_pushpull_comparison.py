...
EPSILONS = [0.1, 0.2, 0.3]              # Additive noise levels
...
def plot_comparison(df):
    ...
    # Plot Uniform Push-Pull line (horizontal line with markers)
    subset = df[df['algorithm'] == 'Uniform Push-Pull'].sort_values('q')
    if len(subset) > 0:
        mean_val = subset.iloc[0]['mean']
        ci_low = subset.iloc[0]['ci_low']
        ci_high = subset.iloc[0]['ci_high']
        
        # Plot horizontal line with markers (matching LA-Pull style)
        ax.plot(subset['q'], [mean_val] * len(subset),
               marker='o', markersize=7, linewidth=2.5,
               color=colors_uniform, linestyle=line_style_uniform,
               label=f'Uniform Push-Pull',
               zorder=2)
        
        # Add shaded CI band
        ax.fill_between(subset['q'], ci_low, ci_high,
                       alpha=0.10, color=colors_uniform, zorder=0)
    ...
    # Legend
    ax.legend(fontsize=10, loc='upper right', framealpha=0.95, ncol=1,
             handlelength=2.5)
